...
def test_real_nht_export_render_round_trip(tmp_path: Path) -> None:
    ...
    _link(
        source / "sfm/reconstruction.json", tmp_path / "sfm/reconstruction.json"
    )
    ...
    assert np.allclose(
        np.asarray(metadata["sfm_to_scene"])
        @ np.asarray(metadata["scene_to_sfm"]),
        np.eye(4),
        atol=1e-6,
    )
    ...
    (tmp_path / "3dgs/training.json").write_text(
        json.dumps(training, indent=2) + "\n"
    )
    scene = create_scene_export(
        tmp_path, "gpu-round-trip", "nht_standard_scene_v1"
    )
    ...
    trainer_canvas = np.asarray(
        Image.open(model / "renders/val_step0_0000.png"), dtype=np.float32
    ) / 255.0
    ...
    assert alpha.shape == depth.shape == (
        observed["height"],
        observed["width"],
        1,
    )
