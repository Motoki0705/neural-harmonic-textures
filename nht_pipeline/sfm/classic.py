...
def run_sift_incremental(
    ...
    if config["camera_sharing"] == "segments":
        extraction_image_dir = _segment_image_root(
            image_dir,
            candidate_dir,
            int(config["camera_segment_size"]),
        )
    ...
    extraction_options.num_threads = int(config["num_threads"])
    extraction_options.use_gpu = False
    sift_extraction = pycolmap.SiftExtractionOptions()
    sift_extraction.max_num_features = int(config["max_features"])
    ...
    pairing_options.overlap = int(config["sequential_overlap"])
    pairing_options.quadratic_overlap = bool(config["quadratic_overlap"])
    pairing_options.loop_detection = False
    pairing_options.num_threads = int(config["num_threads"])
    matching_options = pycolmap.FeatureMatchingOptions()
    matching_options.num_threads = int(config["num_threads"])
    ...
    mapping_options.num_threads = int(config["num_threads"])
    mapping_options.mapper.num_threads = int(config["num_threads"])
